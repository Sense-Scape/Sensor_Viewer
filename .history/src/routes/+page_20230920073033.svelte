		...
		if (typeof window !== 'undefined') {
			...
			if (true) {
				...
				newWebSocket.addEventListener('message', async (event) => {
					...
					console.log(jsonObject['TimeChunk']['Channels']['0']);
					// // Re-render the chart with updated data
					chartCanvas.data.datasets[0].data = jsonObject['TimeChunk']['Channels']['0'];
					chartCanvas.data.labels = Array.from({
						length: jsonObject['TimeChunk']['Channels']['0'].length
					});
					chartCanvas.update();
				});
			}
